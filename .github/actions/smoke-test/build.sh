#!/bin/bash
IMAGE="$1"
VALIDATE_TAGS="${INPUT_VALIDATE_TAGS:-false}"
...
echo "(*) Building image - ${IMAGE}" # --no-cache 
BUILDKIT_PROGRESS=plain devcontainer build --image-name ${id_image} --workspace-folder "src/${IMAGE}/"
